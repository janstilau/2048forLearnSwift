...
class GameboardView: UIView {
    ...
    let tilePopDelay: TimeInterval = 0.05 // 滑动后多长时间insertTile
    let tileExpandTime: TimeInterval = 0.18 // insertTile放大时间
    let titleContractTime: TimeInterval = 0.08 // insertTile缩小时间
    
    let tileMergeStartScale: CGFloat = 1.0
    let tileMergeExpandTime: TimeInterval = 0.08 //合并放大时间
    let tileMergeContractTime: TimeInterval = 0.08 //合并缩小时间
    
    let perSquareSlideDuration: TimeInterval = 0.08 // 滑动时间
    ...
    func setupBackground(backgroundColor bgColor: UIColor, tileColor: UIColor){
        ...
        for _ in 0..<dimension {
            yCursor = tilePadding
            for _ in 0..<dimension{
                let background = UIView.init(frame: CGRect(x: xCursor, y: yCursor, width: tileWidth, height: tileWidth))
    ...
    func insertTile(_ pos: (Int, Int), value: Int) {
        ...
        UIView.animate(withDuration: tileExpandTime, delay: tilePopDelay, options: UIViewAnimationOptions(),
                                   ...
                                   completion: { finished in
                                    ...
                                    UIView.animate(withDuration: self.titleContractTime, animations: { () -> Void in
                                        tile.layer.setAffineTransform(CGAffineTransform.identity)
                                    })
        })
    }
